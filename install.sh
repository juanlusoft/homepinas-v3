...
# NonRAID is NOT compatible with kernel 6.9 or 6.10
if [ "$KERNEL_MAJOR" = "6" ] && ([ "$KERNEL_MINOR" = "9" ] || [ "$KERNEL_MINOR" = "10" ]); then
    NONRAID_COMPATIBLE=false
    echo -e "${YELLOW}⚠ Kernel $KERNEL_VERSION detected - NonRAID not compatible with 6.9/6.10${NC}"
    echo -e "${YELLOW}  Only SnapRAID + MergerFS available.${NC}"
    echo -e ""
fi

# Interactive backend selection
if [ "$NONRAID_COMPATIBLE" = true ]; then
    echo -e "Kernel $KERNEL_VERSION detected - ${GREEN}Compatible with both backends${NC}"
    echo -e ""
    echo -e "${YELLOW}Select storage backend:${NC}"
    echo -e "  ${GREEN}1)${NC} SnapRAID + MergerFS (Recommended for beginners)"
    echo -e "  ${CYAN}2)${NC} NonRAID (Advanced - Real-time parity)"
    echo -e ""
    ...
    # Auto-select SnapRAID when NonRAID is not compatible
    STORAGE_BACKEND="snapraid"
    echo -e "${GREEN}Auto-selected: SnapRAID + MergerFS${NC}"
fi
...
if [ "$STORAGE_BACKEND" = "nonraid" ]; then
    ...
    apt-get install -y $APT_OPTS dkms linux-headers-$(uname -r) build-essential || {
        echo -e "${YELLOW}Note: Some kernel headers may not be available, continuing...${NC}"
    }
    
    # Create NonRAID directories
    mkdir -p /mnt/disk1 /mnt/disk2 /mnt/disk3 /mnt/disk4 /mnt/disk5 /mnt/disk6
    mkdir -p /mnt/parity
    mkdir -p /etc/nonraid
    
    # NonRAID configuration file
    cat > /etc/nonraid/nonraid.conf <<'NRCONF'
# NonRAID Configuration
# Generated by HomePiNAS installer

# Data disks (will be configured via dashboard)
# DATA_DISKS=()

# Parity disk (will be configured via dashboard)
# PARITY_DISK=
...
NRCONF
    ...
    # Create NonRAID systemd service
    cat > /etc/systemd/system/nonraid.service <<'NRSVC'
[Unit]
Description=NonRAID Parity Protection Service
After=local-fs.target

[Service]
Type=oneshot
RemainAfterExit=yes
ExecStart=/usr/local/bin/nonraid-start.sh
ExecStop=/usr/local/bin/nonraid-stop.sh

[Install]
WantedBy=multi-user.target
NRSVC

    # Create NonRAID start script
    cat > /usr/local/bin/nonraid-start.sh <<'NRSTART'
#!/bin/bash
# NonRAID Start Script
# Mounts configured disks and starts parity protection

CONFIG_FILE="/etc/nonraid/nonraid.conf"
DAT_FILE="/nonraid.dat"

if [ ! -f "$DAT_FILE" ]; then
    echo "NonRAID not configured yet. Use dashboard to configure."
    exit 0
fi
...
echo "NonRAID service started"
NRSTART
    chmod +x /usr/local/bin/nonraid-start.sh

    # Create NonRAID stop script
    cat > /usr/local/bin/nonraid-stop.sh <<'NRSTOP'
#!/bin/bash
# NonRAID Stop Script
echo "NonRAID service stopped"
NRSTOP
    chmod +x /usr/local/bin/nonraid-stop.sh

    echo -e "${GREEN}NonRAID installed${NC}"
fi
